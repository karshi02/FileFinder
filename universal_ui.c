...
void universal_show_popup(UniversalIndex *idx) {
    ...
    g_hwnd = CreateWindowExA(
        ...
        "UniversalFinder", "🔍 Search Everything (Type, then ↓ ↑ to select, Enter to open)",
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN: {
        ...
        case VK_RETURN:
            // Enter key - เปิดรายการที่เลือก
            open_selected(hwnd);
            return 0;
            
        case VK_DOWN:
            // ลูกศรลง - เลื่อนไปรายการถัดไป
            if (g_hList && g_result_count > 0) {
                ...
                // ย้าย focus ไปที่ ListBox อัตโนมัติ
                SetFocus(g_hList);
            }
            return 0;
            
        case VK_UP:
            // ลูกศรขึ้น - เลื่อนไปรายการก่อนหน้า
            if (g_hList && g_result_count > 0) {
                int curSel = (int)SendMessageA(g_hList, LB_GETCURSEL, 0, 0);
                if (curSel > 0) {
                    SendMessageA(g_hList, LB_SETCURSEL, curSel - 1, 0);
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == ID_LIST && HIWORD(wParam) == LBN_SELCHANGE) {
            // เมื่อเลือกเปลี่ยน - ไม่ต้องทำอะไร
        }
        break;
...
static void do_search(HWND hwnd, HWND hEdit, HWND hList) {
    ...
    snprintf(debug_title, sizeof(debug_title), "🔍 Found %d results for '%s'", g_result_count, query);
    ...
    for (int i = 0; i < g_result_count; i++) {
        ...
        if (g_results[i]->type && strcmp(g_results[i]->type, "app") == 0) icon = "📱 ";
        else if (g_results[i]->type && strcmp(g_results[i]->type, "program") == 0) icon = "⚙️ ";
        else if (g_results[i]->type && strcmp(g_results[i]->type, "recent") == 0) icon = "📄 ";
        else icon = "📁 ";
